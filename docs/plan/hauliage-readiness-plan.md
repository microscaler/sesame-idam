...
| H1.5 ✅ | [RLS bridge delivery](./hauliage-readiness-tasks/README.md#track-b--transaction-local-rls-h15): validated claims → Lifeguard base-executor transaction context, versioned helper SQL, production policy, and zero-bleed proof | **Done 2026-07-14.** Contract, first `organization_profiles` path, cross-tenant denial, concurrent two-context/100-repeat pool proof, and the live BFF → Company run are delivered without a Sesame-specific executor. The live shared-stack proof resolved the shipper token only to AME Corp and the transporter token only to Transport Services. Session-scoped GUCs are explicitly rejected. |
...
| H7.4 ✅ | RLS zero-bleed E2E: validated Hauliage token → Lifeguard contextual transaction → unqualified query, including pooled reuse, rollback, missing context, and cross-tenant attempts | **Done 2026-07-14.** Focused database tests cover the failure/reuse matrix; the live shared-stack proof covers real Sesame login/JWKS → Hauliage BFF → Company → forced PostgreSQL RLS for both seeded organizations. See the [delivery task list](./hauliage-readiness-tasks/README.md). |
