...
| H1.5 🔶 | [RLS bridge delivery](./hauliage-readiness-tasks/README.md#track-b--transaction-local-rls-h15): validated claims → Lifeguard base-executor transaction context, versioned helper SQL, production policy, and zero-bleed proof | **Implementation complete 2026-07-14.** Contract, first `organization_profiles` path, cross-tenant denial, and concurrent two-context/100-repeat pool proof are delivered without a Sesame-specific executor. Remaining milestone evidence is the live BFF → Company run under D2/H7.4. Session-scoped GUCs are explicitly rejected. |
...
| H7.4 | RLS zero-bleed E2E: validated Hauliage token → Lifeguard contextual transaction → unqualified query, including pooled reuse, rollback, missing context, and cross-tenant attempts | Completion evidence for H1.5; see the [delivery task list](./hauliage-readiness-tasks/README.md) |
