...
|-------|------|--------|----------------------|
| 1.1 | `stories/story-1.1.md` | **Implemented** | `identity-session-service/impl/src/key_manager.rs` (~1067 lines) — `JwtSigningKey` (Ed25519 gen via ring, sign, verify), `KeyManager` (rotation, revocation `revoke_key()` — fully removes key from JWKS and memory, health, find_public_key, kid counter), `JwksDocument` (JWKS format). Handlers: `controllers/jwks.rs` (/.well-known/jwks.json with Cache-Control, X-Content-Type-Options, Vary headers), `controllers/admin_jwks_revoke.rs` (POST /admin/jwks/revoke). Consumer client: `jwks_client.rs` (algorithm allow-list, per-service configs, JWKS poisoning guard, validation pipeline). Static `KEY_MANAGER` LazyLock. 74 tests (14 unit + 12 BDD + 48 key_manager) — **all pass**. Audit logging (`sesame_audit` EMITTER) on key lifecycle events (generate, rotate, revoke, grace_key_expired). **Gates: Compilation PASS, Lint PASS (clippy pedantic), All tests PASS (74/74).** |
