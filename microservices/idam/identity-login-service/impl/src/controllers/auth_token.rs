...
pub fn handle(req: TypedHandlerRequest<Request>) -> Response {
    ...
    if req.data.grant_type == "client_credentials" {
        match handle_client_credentials(&req.data) {
            Ok(resp) => {
                span.record("result", "success");
                emit_audit(true, "client_credentials");
                return resp;
            }
            Err(err) => {
                span.record("result", "denied");
    ...
    // Other grant types: refresh_token rotation lives in
    // identity-session-service /auth/refresh.
    span.record("result", "denied");
...
/// Scope actually granted: the intersection of the requested scope with the
/// client's registered permissions; the full permission set when no scope
/// was requested.
fn granted_scope(requested: Option<&str>, permissions: &[String]) -> Vec<String> {
    match requested {
        None | Some("") => permissions.to_vec(),
        Some(req) => {
            let allowed: HashSet<&str> = permissions.iter().map(String::as_str).collect();
...
/// Access-token TTL for M2M clients (seconds).
const CLIENT_CREDENTIALS_TTL_SECS: i64 = 900;

fn invalid_client(description: &str) -> ErrorResponse {
    ErrorResponse {
        error: "invalid_client".to_string(),
        error_description: description.to_string(),
        retry_after: None,
        hint: Some("Check client_id/client_secret and the X-Tenant-ID header".to_string()),
    }
}

/// Validate an M2M client against the api-keys table and issue an
/// EdDSA-signed access token. Per RFC 6749 §4.4.3 no refresh token is
/// issued (the gen Response requires the field, so it is empty).
fn handle_client_credentials(req: &Request) -> Result<Response, ErrorResponse> {
    use sesame_idam_api_keys::services::api_key_service::{
        decode_permissions, ApiKeyService, ValidationOutcome,
    };
    ...
    let key = match ApiKeyService::validate(tenant_id, client_secret, "any", exec) {
        Ok(ValidationOutcome::Valid(key)) => key,
        Ok(ValidationOutcome::Expired(_)) => {
            return Err(invalid_client("client credentials expired"));
        }
        Ok(_) => return Err(invalid_client("invalid client credentials")),
        Err(e) => {
            tracing::error!(error = %e, "client_credentials: api key lookup failed");
            return Err(ErrorResponse {
                error: "server_error".to_string(),
                error_description: "credential store unavailable".to_string(),
                retry_after: Some(5),
                hint: None,
            });
        }
    ...
    // client_id must match the registered key name (defence in depth — the
    // secret alone already identifies the key, but a mismatched pair is a
    // misconfigured caller and must not silently succeed).
    if key.name != client_id {
        return Err(invalid_client("client_id does not match credential"));
    }

    let permissions = decode_permissions(&key);
    let scope = granted_scope(req.scope.as_deref(), &permissions);
    ...
    let payload = serde_json::json!({
        "iss": std::env::var("SESAME_JWT_ISSUER")
            .unwrap_or_else(|_| "https://idam.example.com".to_string()),
        "sub": format!("client:{}", key.id),
        "client_id": key.name,
        "tenant_id": key.tenant_id,
        "iat": now,
        "nbf": now,
        "exp": now + CLIENT_CREDENTIALS_TTL_SECS,
        "jti": jti,
        "ver": 1,
        "sid": jti,
        "scope": scope.join(" "),
        "sx": { "roles": [], "permissions": permissions },
    });

    let access_token = crate::services::token_issuer::SIGNER
        .sign_payload(&payload.to_string())
        .map_err(|e| ErrorResponse {
            error: "server_error".to_string(),
            error_description: format!("token signing failed: {e}"),
    ...
    Ok(Response {
        access_token,
        token_type: "Bearer".to_string(),
        expires_in: i32::try_from(CLIENT_CREDENTIALS_TTL_SECS).unwrap_or(900),
        refresh_token: String::new(),
        refresh_token_expires_in: None,
        user_id: format!("client:{}", key.id),
        id_token: None,
        mfa_required: None,
        scope: Some(scope.join(" ")),
        entitlements_hash: None,
        entitlements_ref: None,
        permissions: Some(scope),
        roles: None,
...
mod tests {
    ...
    #[test]
    fn test_granted_scope_intersection() {
        let perms = vec!["tenants:write".to_string(), "accounts:write".to_string()];
        // No scope requested → full permission set.
        assert_eq!(granted_scope(None, &perms), perms);
        assert_eq!(granted_scope(Some(""), &perms), perms);
        // Requested ∩ permitted.
        assert_eq!(
            granted_scope(Some("accounts:write admin:god"), &perms),
            vec!["accounts:write".to_string()]
        );
        // Nothing permitted survives.
        assert!(granted_scope(Some("admin:god"), &perms).is_empty());
    }
