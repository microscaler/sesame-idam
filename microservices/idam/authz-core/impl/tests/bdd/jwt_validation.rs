...
fn sign_test_jwt(payload: &str, kid: &str) -> String {
    ...
    let key_pair = km.current_signing_key().expect("KEY_MANAGER must have a current key");
...
fn test_valid_ed25519_jwt_accepted() {
    // Given: a valid JWT signed with current Ed25519 key
    let (jwt, _kid) = create_valid_jwt();

    // When: we call the search_audit_events handler with valid JWT
    ...
    // Then: request is constructible (JWT validation would pass before handler runs)
    // If JWT is invalid, brrtrouter returns 401 BEFORE the handler is called.
    let _body = serde_json::to_value(req.body.clone());
    assert!(_body.is_ok(), "Request with valid Ed25519 JWT should pass JWT validation");
}
...
fn test_missing_auth_token_rejected() {
    ...
    // Then: verify request has no Authorization header
    let has_auth = req
        .headers
        .iter()
...
fn test_malformed_jwt_rejected() {
    // Given: a malformed JWT (not 3 base64 parts)
...
fn test_wrong_algorithm_rejected() {
    // Given: a JWT signed with Ed25519 but claiming HS256 algorithm
    // (simulates algorithm confusion attack)
    let km = KEY_MANAGER.read().unwrap();
    let kid = km
        .current_signing_key()
        .map(|k| k.public_key_jwk.kid.clone())
    ...
    // Sign with Ed25519 but claim HS256 — tests algorithm confusion guard
...
fn test_jwt_without_kid_rejected() {
    // Given: a JWT header without "kid" field
    let km = KEY_MANAGER.read().unwrap();
...
fn test_expired_jwt_rejected() {
    // Given: a JWT signed with Ed25519 but with exp in the past
    let km = KEY_MANAGER.read().unwrap();
    let kid = km
        .current_signing_key()
        .map(|k| k.public_key_jwk.kid.clone())
...
fn test_alg_none_attack_rejected() {
    // Given: a JWT claiming "alg": "none" — classic algorithm confusion attack
    let header = serde_json::json!({
        "alg": "none",
        "typ": "JWT"
    });
    ...
    // alg:none JWTs have empty signature segment
...
fn test_missing_bearer_prefix_rejected() {
    ...
    // Check the Authorization header does not start with "Bearer "
    let auth_header = req
        .headers
        .iter()
...
fn test_valid_token_with_correct_claims() {
    // Given: a JWT with correct iss, aud, exp, and scope
    let (jwt, _kid) = create_valid_jwt();
    ...
    // The request should be constructible (JWT validation passes before handler runs)
    let _body = serde_json::to_value(req.body.clone());
}
...
fn test_jwks_key_available_for_validation() {
    // Given: KEY_MANAGER has a current key
    let km = KEY_MANAGER.read().unwrap();
