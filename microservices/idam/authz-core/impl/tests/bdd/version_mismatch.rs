...
use sesame_idam_authz_core::auth_error::{AuthError, GapSize, MESSAGE_STALE_AUTH_TOKEN};
...
fn test_http_response_401_with_retry_after() {
    ...
    if let Some(obj) = resp.body.as_object() {
        ...
        assert_eq!(
            obj.get("retry_after").and_then(serde_json::Value::as_u64),
            Some(300)
        );
        assert_eq!(
...
fn test_retry_after_header_matches_json_body() {
    ...
    assert_eq!(
        ...
        "Retry-After header ({header_retry}) must match JSON body ({json_retry})"
    );
}
...
///
/// Given: user eve has `cached_ver=150`
/// When: a high-risk request arrives with ver=1 (gap=149 > 100 → large)
/// Then: version mismatch is checked and 401 with `retry_after=0` is returned
#[test]
fn test_stale_token_on_high_risk_route_denied() {
    let result = AuthError::handle_version_mismatch(150, 1);
...
///
/// Given: `cached_ver=200`, claims.ver=10 (gap=190 > 100 → large gap)
...
fn test_concurrent_identical_checks() {
    for _ in 0..50 {
        let result = AuthError::handle_version_mismatch(200, 10);
        assert!(result.is_err());

        let err = result.unwrap_err();
        assert_eq!(err.retry_after(), 0);
        assert_eq!(err.expected_min_version(), 200);
