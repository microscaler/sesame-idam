...
use sesame_denylist::{DenylistCache, DenylistConfig, DenylistMetrics};
...
impl DenylistMiddleware {
    ...
    pub fn new(cache: Arc<DenylistCache>, metrics: Arc<DenylistMetrics>, redis_url: String) -> Self {
        Self {
            cache,
            metrics,
            redis_url,
    ...
    pub fn from_env() -> (Self, DenylistConfig) {
        ...
        let metrics = Arc::new(
            DenylistMetrics::register(&registry)
                .expect("Failed to register denylist metrics"),
        );
    ...
    /// Check if a JTI is revoked. Synchronous only (cache-only).
    ///
    /// Returns true if the token is revoked (cache hit), false otherwise.
    /// On cache miss, Redis is consulted via the callback.
    pub async fn check_revocation(&self, jti: &str, token_exp_epoch: Option<u64>) -> bool {
        ...
        match result {
            sesame_denylist::DenylistResult::CacheHit => {
                self.metrics.inc_hits();
                true
            }
            sesame_denylist::DenylistResult::RedisHit => {
                self.metrics.inc_misses();
                self.metrics.inc_redis_hits();
                true
            }
            sesame_denylist::DenylistResult::RedisMiss => {
                self.metrics.inc_misses();
                self.metrics.inc_redis_misses();
                false
            }
            sesame_denylist::DenylistResult::RedisUnavailable => {
                self.metrics.inc_misses();
...
impl Middleware for DenylistMiddleware {
    fn before(&self, req: &HandlerRequest) -> Option<HandlerResponse> {
        ...
        if let Some(jti) = jti {
            ...
            if self.cache.is_present(jti) {
                let res = HandlerResponse::json(401, serde_json::json!({
                    "error": "token_revoked",
...
impl DenylistMiddlewareBuilder {
    ...
    pub fn build(self) -> DenylistMiddleware {
        ...
        let cache = self.cache.unwrap_or_else(|| Arc::new(DenylistCache::new(config.clone())));
