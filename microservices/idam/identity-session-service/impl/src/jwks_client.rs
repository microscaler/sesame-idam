...
/// HS256 and `alg: none` are explicitly rejected.
...
pub enum ValidationResult {
    Valid,
    InvalidTyp { expected: String, got: String },
    InvalidAlgorithm { expected: Vec<&'static str>, got: String },
    MissingKkid,
    KeyNotFound { kid: String },
    InvalidSignature,
    InvalidIssuer { expected: Vec<String>, got: String },
    InvalidAudience { expected: Vec<String>, got: String },
    Expired { exp: i64, now: i64, leeway: i64 },
    NotBefore { nbf: i64, now: i64, leeway: i64 },
    MissingSubject,
    JtiRevoked,
}
...
impl ValidationResult {
    ...
    pub fn reason(&self) -> &str {
        match self {
            ...
            ValidationResult::MissingKkid => "kid",
            ValidationResult::KeyNotFound { .. } => "kid",
...
impl Default for JwksServiceConfig {
    fn default() -> Self {
        Self {
            ...
            cache_ttl_secs: 300,
            leeway_secs: 60,
            allowed_algorithms: ALLOWED_JWT_ALGORITHMS.iter().map(|s| s.to_string()).collect(),
        }
    }
}
...
impl JwksServiceConfig {
    ...
    /// Return the full OpenID Discovery URL.
...
impl JwksProviderBuilder {
    ...
    pub fn build_with_config(&self) -> (JwksBearerProvider, JwksServiceConfig) {
        ...
        let mut builder = JwksProviderBuilder { config: config.clone() };
...
mod tests {
    ...
    fn test_healthy_health_check() {
        let hc = JwksHealthCheck::healthy(2, vec!["key-1".into(), "key-2".into()], Some(1700000000));
        assert!(hc.reachable);
