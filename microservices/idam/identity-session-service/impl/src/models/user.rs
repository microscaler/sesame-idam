...
/// canonical definition) so session endpoints (`/identity/me`, userinfo) can
/// read the shared `sesame_idam.users` table. Shared entities are duplicated
/// per service by convention; the migrator merges identical table
/// definitions by name.
///
/// Keep in sync with
