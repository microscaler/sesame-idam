...
impl WindowBucket {
    ...
    fn retry_after(&self, window: Duration) -> u64 {
        if let Some(&oldest) = self.timestamps.front() {
            let cutoff = oldest + window;
            let remaining = cutoff.checked_duration_since(Instant::now());
            match remaining {
                Some(r) if !r.is_zero() => r.as_secs().max(1),
                _ => 0,
            }
        } else {
            0
...
///
/// Uses a `Mutex<HashMap>` for thread-safe concurrent access.
/// Keys are either tenant IDs (from `X-Tenant-ID`) or `"global"` for
/// unauthenticated traffic.
#[derive(Clone)]
pub struct RateLimiterState {
    /// Per-key sliding window buckets.
    buckets: std::sync::Arc<std::sync::Mutex<std::collections::HashMap<String, WindowBucket>>>,
...
impl RateLimiterState {
    ...
    fn check_limit(
        ...
        if count >= config.max_requests {
            // Re-lock to record retry_after
            let retry_after = {
                let mut guard = self.buckets.lock().unwrap();
                let bucket = guard.get_mut(key).unwrap();
                bucket.retry_after(window)
            };
            tracing::warn!(
