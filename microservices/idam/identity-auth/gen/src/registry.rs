
// ⚠️ WARNING: This file is auto-generated by BRRTRouter
// ⚠️ DO NOT MODIFY - Changes will be overwritten on next generation
// ⚠️ To modify API behavior, edit the OpenAPI spec and regenerate
// ⚠️ To implement business logic, edit the corresponding controller file
use brrtrouter::dispatcher::Dispatcher;
use brrtrouter::spec::RouteMeta;
use brrtrouter::typed::spawn_typed_with_stack_size_and_name;
// Note: avoid wildcard imports to reduce warnings

/// # Safety
/// This function spawns handler coroutines. Callers must ensure coroutine runtime is set up.
#[allow(dead_code)]
pub unsafe fn register_all(dispatcher: &mut Dispatcher) {
    
    dispatcher.register_typed_with_stack_size(
        "jwks",
        crate::controllers::jwks::JwksController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "openid_configuration",
        crate::controllers::openid_configuration::OpenidConfigurationController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "query_users",
        crate::controllers::query_users::QueryUsersController,
        28672,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "create_user",
        crate::controllers::create_user::CreateUserController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "fetch_user_by_email",
        crate::controllers::fetch_user_by_email::FetchUserByEmailController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "users_me_get",
        crate::controllers::users_me_get::UsersMeGetController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "users_me_patch",
        crate::controllers::users_me_patch::UsersMePatchController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "migrate_user",
        crate::controllers::migrate_user::MigrateUserController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "migrate_user_passwords",
        crate::controllers::migrate_user_passwords::MigrateUserPasswordsController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "fetch_user_by_username",
        crate::controllers::fetch_user_by_username::FetchUserByUsernameController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "get_user",
        crate::controllers::get_user::GetUserController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "update_user",
        crate::controllers::update_user::UpdateUserController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "delete_user",
        crate::controllers::delete_user::DeleteUserController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "disable_user",
        crate::controllers::disable_user::DisableUserController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "update_user_email",
        crate::controllers::update_user_email::UpdateUserEmailController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "verify_user_email",
        crate::controllers::verify_user_email::VerifyUserEmailController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "fetch_employee",
        crate::controllers::fetch_employee::FetchEmployeeController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "enable_user",
        crate::controllers::enable_user::EnableUserController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "logout_all_sessions",
        crate::controllers::logout_all_sessions::LogoutAllSessionsController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "create_magic_link",
        crate::controllers::create_magic_link::CreateMagicLinkController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "disable_user_mfa",
        crate::controllers::disable_user_mfa::DisableUserMfaController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "setup_user_mfa_totp",
        crate::controllers::setup_user_mfa_totp::SetupUserMfaTotpController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "verify_user_mfa",
        crate::controllers::verify_user_mfa::VerifyUserMfaController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "update_user_password",
        crate::controllers::update_user_password::UpdateUserPasswordController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "clear_user_password",
        crate::controllers::clear_user_password::ClearUserPasswordController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "setup_user_phone",
        crate::controllers::setup_user_phone::SetupUserPhoneController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "verify_user_phone",
        crate::controllers::verify_user_phone::VerifyUserPhoneController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "resend_email_confirmation",
        crate::controllers::resend_email_confirmation::ResendEmailConfirmationController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "link_social_account",
        crate::controllers::link_social_account::LinkSocialAccountController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "fetch_user_oauth_tokens",
        crate::controllers::fetch_user_oauth_tokens::FetchUserOauthTokensController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "fetch_fresh_oauth_token",
        crate::controllers::fetch_fresh_oauth_token::FetchFreshOauthTokenController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "list_mcp_agents",
        crate::controllers::list_mcp_agents::ListMcpAgentsController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "register_mcp_agent",
        crate::controllers::register_mcp_agent::RegisterMcpAgentController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "get_mcp_agent",
        crate::controllers::get_mcp_agent::GetMcpAgentController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "deregister_mcp_agent",
        crate::controllers::deregister_mcp_agent::DeregisterMcpAgentController,
        20480,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "auth_forgot_password",
        crate::controllers::auth_forgot_password::AuthForgotPasswordController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "auth_login",
        crate::controllers::auth_login::AuthLoginController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "auth_logout",
        crate::controllers::auth_logout::AuthLogoutController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "auth_refresh",
        crate::controllers::auth_refresh::AuthRefreshController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "auth_register",
        crate::controllers::auth_register::AuthRegisterController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "auth_reset_password",
        crate::controllers::auth_reset_password::AuthResetPasswordController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "auth_token",
        crate::controllers::auth_token::AuthTokenController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "mcp_token_exchange",
        crate::controllers::mcp_token_exchange::McpTokenExchangeController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "mcp_validate_token",
        crate::controllers::mcp_validate_token::McpValidateTokenController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "oauth_authorize",
        crate::controllers::oauth_authorize::OauthAuthorizeController,
        24576,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "oauth_logout",
        crate::controllers::oauth_logout::OauthLogoutController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "oauth_userinfo",
        crate::controllers::oauth_userinfo::OauthUserinfoController,
        16384,
    );
    
    
    dispatcher.register_typed_with_stack_size(
        "social_login",
        crate::controllers::social_login::SocialLoginController,
        20480,
    );
    
    
}

/// Dynamically register handlers for the provided routes using their handler names.
/// # Safety
/// This function spawns handler coroutines. Callers must ensure coroutine runtime is set up.
/// 
/// **IMPORTANT**: This function will clear ALL existing handlers before registering new ones
/// to prevent memory leaks from accumulating coroutines.
pub unsafe fn register_from_spec(dispatcher: &mut Dispatcher, routes: &[RouteMeta]) {
    // Clear all existing handlers to prevent memory leaks
    // The old senders will be dropped, causing their coroutines to exit
    dispatcher.handlers.clear();
    
    // Also clear worker pools to prevent resource leaks from previous pool-based registrations
    dispatcher.worker_pools.clear();
    
    for route in routes {
        // JSF P0-2: Use as_ref() for Arc<str> -> &str conversion
        match route.handler_name.as_ref() {
            "jwks" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::jwks::JwksController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "openid_configuration" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::openid_configuration::OpenidConfigurationController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "query_users" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::query_users::QueryUsersController,
                    28672,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "create_user" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::create_user::CreateUserController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "fetch_user_by_email" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::fetch_user_by_email::FetchUserByEmailController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "users_me_get" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::users_me_get::UsersMeGetController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "users_me_patch" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::users_me_patch::UsersMePatchController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "migrate_user" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::migrate_user::MigrateUserController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "migrate_user_passwords" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::migrate_user_passwords::MigrateUserPasswordsController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "fetch_user_by_username" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::fetch_user_by_username::FetchUserByUsernameController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "get_user" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::get_user::GetUserController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "update_user" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::update_user::UpdateUserController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "delete_user" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::delete_user::DeleteUserController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "disable_user" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::disable_user::DisableUserController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "update_user_email" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::update_user_email::UpdateUserEmailController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "verify_user_email" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::verify_user_email::VerifyUserEmailController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "fetch_employee" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::fetch_employee::FetchEmployeeController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "enable_user" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::enable_user::EnableUserController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "logout_all_sessions" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::logout_all_sessions::LogoutAllSessionsController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "create_magic_link" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::create_magic_link::CreateMagicLinkController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "disable_user_mfa" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::disable_user_mfa::DisableUserMfaController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "setup_user_mfa_totp" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::setup_user_mfa_totp::SetupUserMfaTotpController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "verify_user_mfa" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::verify_user_mfa::VerifyUserMfaController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "update_user_password" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::update_user_password::UpdateUserPasswordController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "clear_user_password" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::clear_user_password::ClearUserPasswordController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "setup_user_phone" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::setup_user_phone::SetupUserPhoneController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "verify_user_phone" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::verify_user_phone::VerifyUserPhoneController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "resend_email_confirmation" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::resend_email_confirmation::ResendEmailConfirmationController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "link_social_account" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::link_social_account::LinkSocialAccountController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "fetch_user_oauth_tokens" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::fetch_user_oauth_tokens::FetchUserOauthTokensController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "fetch_fresh_oauth_token" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::fetch_fresh_oauth_token::FetchFreshOauthTokenController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "list_mcp_agents" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::list_mcp_agents::ListMcpAgentsController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "register_mcp_agent" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::register_mcp_agent::RegisterMcpAgentController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "get_mcp_agent" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::get_mcp_agent::GetMcpAgentController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "deregister_mcp_agent" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::deregister_mcp_agent::DeregisterMcpAgentController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "auth_forgot_password" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::auth_forgot_password::AuthForgotPasswordController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "auth_login" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::auth_login::AuthLoginController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "auth_logout" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::auth_logout::AuthLogoutController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "auth_refresh" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::auth_refresh::AuthRefreshController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "auth_register" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::auth_register::AuthRegisterController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "auth_reset_password" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::auth_reset_password::AuthResetPasswordController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "auth_token" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::auth_token::AuthTokenController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "mcp_token_exchange" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::mcp_token_exchange::McpTokenExchangeController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "mcp_validate_token" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::mcp_validate_token::McpValidateTokenController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "oauth_authorize" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::oauth_authorize::OauthAuthorizeController,
                    24576,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "oauth_logout" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::oauth_logout::OauthLogoutController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "oauth_userinfo" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::oauth_userinfo::OauthUserinfoController,
                    16384,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            "social_login" => {
                
                let tx = spawn_typed_with_stack_size_and_name(
                    crate::controllers::social_login::SocialLoginController,
                    20480,
                    Some(route.handler_name.as_ref()),
                );
                dispatcher.add_route(route.clone(), tx);
                
            }
            
            _ => {}
        }
    }
}