...
# Lints the impl crates + database + migrator (strict: -D warnings).
# sesame-common is linted separately via lint-common (Phase 1: warn) until its
# pedantic backlog (~380 findings, mostly missing `# Errors` docs) is cleared.
lint-rust:
...
# Phase 1 (warn) pedantic lint for the shared common crate. Promote to
# -D warnings (and fold into lint-rust) once the pedantic backlog is cleared.
lint-common:
    @echo "🔍 Linting sesame-common (Phase 1: warnings only)..."
    @cd microservices && cargo clippy --all-targets --all-features --no-deps \
      -p sesame-common \
      -- -W clippy::pedantic
